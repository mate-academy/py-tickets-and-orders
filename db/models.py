...
class Ticket(models.Model):
    ...
    def clean(self) -> None:
        # Created these variables to fix E131 (flake8 error)

        error_text_for_row = {"row": f"row number must be in available range: "
                              f"(1, rows): (1, "
                              f"{self.movie_session.cinema_hall.rows})"}
        error_text_for_seat = {"seat": f"seat number must be in available "
                               f"range: (1, seats_in_row): (1, "
                               f"{self.movie_session.cinema_hall.seats_in_row}"
                               f")"}

        if self.row > self.movie_session.cinema_hall.rows:
            raise ValidationError(error_text_for_row)

        if self.seat > self.movie_session.cinema_hall.seats_in_row:
            raise ValidationError(error_text_for_seat)
