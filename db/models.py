...
class Ticket(models.Model):
    ...
    def clean(self) -> None:
        number_of_rows = self.movie_session.cinema_hall.rows
        number_of_seats = self.movie_session.cinema_hall.seats_in_row

        if not (1 <= self.row <= number_of_rows):
            raise ValidationError({
                "row": f"row number must be in available range: "
                       f"(1, rows): (1, {number_of_rows})"
            })
