...
class Ticket(models.Model):
    ...
    class Meta:
        constraints = [
            UniqueConstraint(fields=["row", "seat"],
    ...
    def clean(self) -> None:
        if self.row <= 0 or self.row > self.movie_session.cinema_hall.rows:
            error_string = (f"row number must be in "
                            f"available range: "
                            f"(1, rows): "
                            f"(1, {self.movie_session.cinema_hall.rows})")
            raise ValidationError({"row": [error_string]})
        if (self.seat <= 0
                or self.seat > self.movie_session.cinema_hall.seats_in_row):
            error_string = (f"seat number must be in "
                            f"available range: "
                            f"(1, seats_in_row): (1, "
                            f"{self.movie_session.cinema_hall.seats_in_row})")
            raise ValidationError({"seat": [error_string]})
