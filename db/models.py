    ...
    class Meta:
        constraints = [
            UniqueConstraint(
                ...
                name="ticket__row__seat__movie_session")
        ]
    ...
    def clean(self):
        if not 1 <= self.row <= self.movie_session.cinema_hall.rows:
            raise ValidationError(
                f"Row should be in range [{1};{self.movie_session.cinema_hall.rows}], "
                f"not {self.row}!!!"
            )
        if not 1 <= self.seat <= self.movie_session.cinema_hall.seats_in_row:
            raise ValidationError(
                f"Seat should be in range [{1};{self.movie_session.cinema_hall.seats_in_row}],"
                f"not {self.seat}!!!"
            )
